...
namespace ns_searcher
{
    ...
    public:
        ...
        void Search(const string &query, string &json_string)
        {
            ...
            //[2]根据分词结果，查找到排拉链
            vector<ns_index::InvertedElem> invertedlistall; // 把所有的倒排节点汇总在一起
            for (string &word : queryret)
            {
                // 这个地方获得了关键字，就要获得对应的倒排拉链
                vector<ns_index::InvertedElem> *list = index->GetInvertedlist(word);
            ...
            sort(invertedlistall.begin(), invertedlistall.end(), [](const ns_index::InvertedElem& n1,const ns_index::InvertedElem& n2) {
                return n1.weight>n2.weight;//保持前面大于后面,降序排序
            });
            // 【4】构建json串            
            for(ns_index::InvertedElem& item:invertedlistall)
            {
